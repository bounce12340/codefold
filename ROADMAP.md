...
6. **coverage 缺口暗區** — 值得做：讓從未被測試走過的函式一眼可見，直接暴露測試盲點。**難度：低至中**。
...
15. **解除 preview 測試與既有 build 產物耦合** — 值得做：目前 `tests/previewHarness.test.ts` 讀 `tools/preview/index.html`，改 src 後未 build 會假性失敗；測試內呼叫同一 build helper 產生暫存 HTML 可消除此陷阱。**難度：低**。
