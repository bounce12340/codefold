...
7. ~~**依賴健康**~~ — **已完成（第一階段）**。`src/graph/dependencyHealth.ts` 以 Tarjan 強連通分量偵測 import 循環，隨 `WebviewGraph.dependencyHealth` 送到 webview（靜態結構，不走 `stateUpdate`）。**只分析檔案層 import 邊**——函式層的遞迴與互相遞迴是正常寫法，納入會用誤報淹沒真正的 import 循環。呈現：循環邊虛線化、成員卡片檔名後綴 `↺`、`aria-label` 併入「in an import cycle」，側欄以選中檔案為起點展開整圈路徑並顯示 fan-in/fan-out。Tarjan 刻意寫成迭代版，因為遞迴版在 `MAX_FILES`（2000）長鏈下會受制於 JS stack 上限；已用 2000 節點單一強連通分量的測試釘住。

    **未涵蓋**：ROADMAP 原文的「過度耦合簇警示」目前只呈現 fan-in/fan-out 數字而不下判斷——耦合過高的門檻高度依專案而異，武斷設一個值會產生大量無意義警示。群組層（資料夾之間）的循環也尚未偵測，收合狀態下看不到檔案層循環；若要做需先決定群組循環與檔案循環的語意關係。**難度：中**。
...
- **第二層｜放大既有投資**：14（獨立檢視器——`tools/preview/` 已證明畫布可脫離 VS Code）、13（測試事件序列）。〔7 已完成第一階段，剩餘的耦合門檻與群組層循環見該項〕
